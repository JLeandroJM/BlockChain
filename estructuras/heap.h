...
#ifndef PROYECTO_PROYECTO_GRUPO_4_HEAP_H
#define PROYECTO_PROYECTO_GRUPO_4_HEAP_H
...
private:
    ...
    void topRangeR(int index, DoubleList<T> &dl) const;
};
...
template<typename T, bool activate>
DoubleList<T> Heap<T, activate>::topRange()const{
    DoubleList<T> dl;
    topRangeR(0, dl);
    return dl;
}
...
template<typename T, bool activate>
void Heap<T, activate>::topRangeR(int index, DoubleList<T> &dl)const {
...
void Heap<T, activate>::heapifyDown(int index) {
    ...
    int largest = index;
    if (left < size && greater(A[left], A[index]) ) { largest = left; }
    if (right < size && greater(A[right], A[largest]) ) { largest = right; }
    if (largest != index) {
...
void Heap<T, activate>::heapifyUp(int index) {
    if (index && less(A[PARENT(index)], A[index]) ) {
...
#endif //PROYECTO_PROYECTO_GRUPO_4_HEAP_H
